...
만들면 이전 실행 기록은 새 상태 세대에 들어가지 않습니다.  Dojang은 저장소 상태
세대를 확인하면서 프로세스를 시작하지만, 프로세스가 끝나기를 기다리기 전에는
저장소 상태 잠금을 해제합니다.  따라서 명시적으로 허용한 안쪽 호출도 같은 잠금을
얻을 수 있습니다.  실행 기록을 쓰기 전에 훅 실행 식별자와 정책 데이터가 검증된
타입으로 표현됩니다.
