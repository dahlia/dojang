...
역슬래시 경로, 링크, 대소문자 접기 또는 유니코드 정규화 후 충돌하는 항목은
거부합니다.  POSIX 시스템에서는 실행 권한을 포함하여 아카이브에 저장된 권한
비트를 대상 파일시스템이 지원할 때 복원합니다.  지원하지 않는 경우에는 내용을
게시하고 권한을 복원하지 못했다는 경고를 출력합니다.
