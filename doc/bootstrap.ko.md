...
디렉터리에서 진행됩니다.  Dojang은 준비된 *dojang.toml*을 확인한 뒤 저장소를
게시하므로, 올바르지 않거나 불완전한 원본이 대상 디렉터리를 대체하지 않습니다.
...
교환합니다.  지원하지 않으면 기존 디렉터리의 정체성을 유지하고 각 항목을 기존
항목을 대체하지 않는 방식으로 만듭니다.  현재 디렉터리도 정체성을 유지하며 같은
항목별 게시 방식을 사용합니다.  다른 프로세스가 항목을 먼저 만들면 덮어쓰지
않고 실패하며, 정리할 때도 bootstrap이 만든 뒤 바뀌지 않은 항목만 제거합니다.
