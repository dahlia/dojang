...
게시 전에 대상이 심볼릭 링크로 바뀌었다면 거부합니다.  그 밖의 기존 빈 대상
디렉터리는 원자적으로 대체합니다.  게시 도중 다른 프로세스가 대상 디렉터리를
만들면 이를 대체하지 않고 실패합니다.  현재 디렉터리는 기존 디렉터리 정체성을
유지합니다.  대상 전체를 교체해 게시하려면 대상 파일시스템이 기존 항목을
대체하지 않는 원자적 rename을 지원해야 합니다.  이 기능을 사용할 수 없으면
경쟁 조건이 있는 rename으로 대체하지 않고 안전하게 실패합니다.  현재
디렉터리에 게시할 때는 각 항목을 기존 항목을 대체하지 않는 방식으로 만듭니다.
다른 프로세스가 항목을 먼저 만들면 덮어쓰지 않고 실패하며, 정리할 때도
bootstrap이 만든 항목만 제거합니다.
...
경우에도 정규 파일로 바꾸지 않고 거부합니다.  POSIX 시스템에서는 실행 권한을
포함하여 아카이브에 저장된 권한 비트를 대상 파일시스템이 지원할 때 복원합니다.
