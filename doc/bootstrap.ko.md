...
실행 권한을 포함하여 아카이브에 저장된 권한 비트를 대상 파일시스템이 지원할 때
복원합니다.  파일시스템 호출이 성공했다는 사실만으로 모든 권한 비트가
복원되었다고 간주하지 않고, 적용 후의 권한을 검사합니다.  정확히 복원할 수 없는
경우에는 내용을 게시하고 권한을 복원하지 못했다는 경고를 출력합니다.  기존의 빈
목적지를 교체할 때는 루트 디렉터리의 원래 권한을 유지합니다.
