...
위의 간결 문법은 이름 붙은 모니커만으로 라우트를 표현할 수 있을 때 유용합니다.
상세 라우트에서는 환경 서술어를 분기의 `when` 필드에 직접 쓸 수 있습니다.
...
상세 라우트는 순서가 있는 배열입니다.  각 분기에는 `moniker`와 `when` 중 정확히
하나만 있어야 하며, `path`를 둘 수 있습니다.  `moniker` 분기는 같은 선언 파일에
정의된 모니커를 참조합니다.  `when` 분기에서는 [환경
서술어](environment-predicate.ko.md)의 모든 문법을 쓸 수 있습니다.  `path`를
생략하면 해당 분기는 공 라우트가 됩니다.  이와 달리 `path = ""`는 빈 경로
표현식입니다.

분기는 나열된 순서대로 검사하며 처음 일치한 분기가 대상 경로를 결정합니다.
이 순서는 명시적이므로, 뒤에 있는 분기의 서술어가 더 구체적이더라도 앞으로
이동하지 않습니다.  간결 라우트에는 아래에서 설명하는 구체성 규칙을 적용합니다.

인라인 서술어가 더 명확할 때, 여러 조건이 같은 모니커에 대응될 때, 또는 반복된
분기를 서로 구별해 유지해야 할 때 상세 형식을 쓰세요.  기존 간결 형식도 계속
유효하며 단순한 모니커 전용 라우트에 적합합니다.


라우팅 우선순위
---------------

간결 라우트에서 현재 환경과 일치하는 항목이 여러 개일 경우, Dojang은
**구체성**(specificity)을 기준으로 어떤 항목을 사용할지 결정합니다.  서술어가
구체적일수록 우선순위가 높습니다.  이를 통해 특수한 설정이 일반적인 설정보다
먼저 적용됩니다.  상세 라우트에는 위에서 설명한 명시적 분기 순서를 적용합니다.
