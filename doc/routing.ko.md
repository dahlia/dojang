...
배포 링크는 의도적으로 단방향입니다.  대상을 통한 읽기와 쓰기가 곧 저장소
원본에 대한 것이므로, 대상을 편집하는 일이 곧 원본을 편집하는 일이고 반영할
것이 없습니다.  `dojang reflect`는 `--force`를 쓰더라도, 링크된 디렉터리
아래의 경로를 지정하더라도 배포 링크를 무조건 거부합니다.  `dojang apply`는
없는 링크를 만들고, (예를 들어 저장소 체크아웃이 이동한 뒤) 대상이 더 이상
원본과 일치하지 않는 링크를 고치며, 대상 위치에 이미 있는 일반 파일이나
디렉터리는 `--force`로 대체하지 않는 한 충돌로 취급합니다.
...
모든 대상 항목은 정확히 하나의 경로 규칙이 소유합니다.  두 경로 규칙의
대상이 중첩되면 — 예를 들어
*$XDG_CONFIG_HOME*을 대상으로 하는 디렉터리 경로 규칙과 *$XDG\_CONFIG\_HOME/nvim*을
대상으로 하는 더 구체적인 경로 규칙 — 더 구체적인 대상을 가진 경로 규칙이 그
하위 트리를 소유하며, 더 넓은 경로 규칙은 제거를 포함해 그 안의 항목을 읽거나
쓰지 않습니다.
...
전에 종료 코드 38로 거부됩니다:
 ...
 -  두 활성 경로 규칙이 어휘적으로든 심볼릭 링크를 해석한 뒤로든 같은 대상
    경로로 전개되는 경우;
 -  대상이 다른 경로 규칙의 배포 링크 안에 있는 경우;
 -  대상이 저장소 체크아웃 자체와 겹치거나, 심볼릭 링크를 통해 자신의
    원본과 겹치는 경우.
