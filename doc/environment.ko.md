...
**환경**(environment)은 설정 파일을 적용할 기기를 구분하는 [머신 정보]의
모음입니다.  Dojang은 **운영체제**(`os`), **프로세서 아키텍처**(`arch`),
**커널**(`kernel`, `kernel-release`), **호스트 이름**(`hostname`)을
감지합니다.[^1]  저장소별 머신 정보를 직접 선언할 수도 있습니다.  예를 들어,
...
이 파일은 환경 시뮬레이션이며 등록한 머신 정보보다 우선합니다.  저장소별 기기
정체성을 영속하려면 `dojang init`에 `--fact`나 `--facts-file`을 사용하세요.
자세한 내용은 [머신 정보]를 참고하세요.
