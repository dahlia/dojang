...
상세 분기의 `codec` 필드는 일반 원본 파일을 배치 전에 변환합니다.  문자열 형식은
설정 없는 코덱의 이름을 지정하고, 객체 형식은 `name`과 `config` 필드를
사용합니다.  기본 `identity` 코덱은 바이트를 그대로 유지하므로 기존의 간결·상세
경로 규칙은 이전과 똑같이 동작합니다.  코덱 메타데이터에는 `path`가 필요하며,
