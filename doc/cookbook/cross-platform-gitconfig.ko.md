...
이를테면 이름이나 자주 쓰는 명령 별칭 등은 모든 기기에서 동일하게 사용하고,
...
이를 위해선 파일별 경로 지정을 통해 환경별 설정 파일이 *~/.gitconfig*에
...
환경 하나에 하나의 파일별 경로 규칙을 만든 것에 주목하세요.
*gitconfig/.gitconfig.mac*의 경로 규칙을 보면 `mac` 모니커를 만족할 경우의
경로만 적혀 있고, `win` 모니커가 만족했을 때의 경로는 누락되어 있습니다.
이렇게 의도적으로 특정 환경에 대해서 누락시키는 것을
[공 경로 규칙](../routing.ko.md#공-경로-규칙)이라고 합니다.
공 경로 규칙을 사용하면 특정 환경에 대해서는 경로 규칙이 존재하지 않는 것으로
...
파일에 아래와 같이 파일별 경로 규칙을 하나 더 추가하는 것을 잊으면 안 됩니다.
