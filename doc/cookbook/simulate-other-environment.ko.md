...
`--dry-run` 옵션은 `dojang` 명령이 수행하는 모든 종류의 파일 시스템 변경을
완전히 샌드박스 안에서 모의 실행하게 합니다.
이 옵션을 쓰면 실제로 파일 시스템에 변경을 가하지 않기에,
어떠한 명령을 실행하더라도 안전합니다.  따라서,
`--dry-run` 옵션을 쓰면 Dojang이 어떤 파일을 어떻게 변경할 것인지를
미리 확인할 수 있습니다.

두 기능 모두 `dojang`의 어떤 하위 명령과 함께 쓸 수 있습니다.
단, `--dry-run` 옵션은 전역적이므로 하위 명령 앞에 써야 합니다.
즉, 아래와 같이 쓰는 대신…
