...
Dojang은 전체 중간 스냅샷을 저장소 밖의 플랫폼별 사용자 데이터 디렉터리에
보관합니다.  새 저장소에는 `dojang init`을 실행할 때 안정적인
`repository-id`가 생기므로 *.gitignore* 항목을 자동 생성할 필요가 없습니다.

이전 Dojang 릴리스에서 만든 저장소에는 작업 트리 안에 *.dojang/* 디렉터리가
남아 있을 수 있습니다.  `dojang migrate`를 실행하면 스냅샷을 복사하고 검증한
뒤 기존 디렉터리를 제거합니다.  플랫폼별 경로, dry-run 절차, 오버라이드 동작,
복구 방법은 [머신 상태] 문서를 참고하세요.
