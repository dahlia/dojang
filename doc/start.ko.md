...
동기화하는 것을 **반영**이라고 부릅니다.  적용과 반영은 `dojang` 명령을 통해
수행할 수 있습니다.
...
선언 파일을 만들려면 기존 항목을 대체하지 않는 원자적 rename을 지원하는 파일
시스템이 필요합니다.  저장소 파일 시스템이 이를 지원하지 않으면 동시에 만들어진
선언 파일을 교체하는 위험을 감수하는 대신 `dojang init`이 파일 쓰기 오류로
중단됩니다.  저장소를 이를 지원하는 로컬 파일 시스템으로 옮긴 뒤 다시
실행하세요.
...
저장소를 초기화하고 나면, `dojang status` 명령을 통해 현재 상황을 확인할 수
있습니다.  이 명령은 현재 기기의 설정 파일들과 저장소 내 설정 파일들을
...
[**경로 지정**](routing.ko.md)은 환경에 따라 설정 파일들을 다른 위치로 복사하는
것을 말합니다.  예를 들어, 홈 디렉터리의 위치는 모든 운영체제에서 다르기 때문에,
홈 디렉터리에 있는 설정 파일들을 각 운영체제에 맞게 다른 위치로 복사해야 합니다.

선언 파일의 내용 중 가장 중요한 것이 바로 이 경로 지정이라고 해도 과언이
아닙니다.  경로 규칙은 선언 파일 내 `dirs` 및 `files` 구획에 기술합니다.
`dojang init` 명령을 통해 저장소를 초기화하면, 이미 선언 파일 내에 기본적인
경로 규칙이 기술되어 있을 것입니다. 예를 들어 다음은 홈 디렉터리에 위치한 설정
파일들을 위한 경로 규칙입니다.
...
`HOME`은 경로 규칙 이름으로, 저장소 내 \*HOME/\*이라는 디렉터리에 들어있는 설정
파일들이 원본이 됩니다.  경로 규칙 이름은 얼마든지 다른 이름이 될 수 있습니다.
...
경로 규칙은 Linux 또는 macOS인 경우 환경 변수 `HOME`이 가리키는 경로로 *HOME/*
...
자세한 내용은 [경로 지정](routing.ko.md) 문서를 참고하세요.
...
등록되지 않은 파일에 여러 경로 규칙이 일치할 수 있는 경우, Dojang은 어떤 경로
규칙을 사용할지 선택하도록 프롬프트를 표시합니다.
...
대상 경로에 여러 경로 규칙이 일치할 수 있는 경우, Dojang은 어떤 경로 규칙을
...
뒤 기존 디렉터리를 제거합니다.  플랫폼별 경로, 모의 실행 절차, 덮어쓰기 동작,
복구 방법은 [머신 상태] 문서를 참고하세요.

성공한 적용과 반영 작업은 관리한 대상도 기록합니다.  선언 파일 변경으로 예전
대상이 고아 레코드가 되면 `dojang status`가 그 이유와 변경 없음·수정됨·사라짐
상태를 표시합니다.  대상 파일을 삭제하지 않고 선택한 고아 레코드만 버리려면
`dojang unmanage`를 사용하세요.  원본과 대상 파일을 모두 보존하면서 이 저장소의
머신별 로컬 상태를 제거하려면 `dojang forget`을 사용하세요.  안전 규칙과
`--force`가 필요한 경우는 [머신 상태] 문서를 참고하세요.
...
해당 줄은 순서대로 다음과 같은 의미를 지닙니다.
 ...
 -  `F`: 원본 설정 파일이 (디렉터리나 심볼릭 링크가 아닌) 일반 파일이다.
