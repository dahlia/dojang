...
record.  Schema version 6 can add redacted codec metadata to a managed-target
record: the codec name and version, configuration digest, cache key, and
dependency fingerprints.  It stores neither source bytes nor another copy of
rendered bytes.  Version 5 documents remain readable and default to the
`identity` codec without cache metadata.  Schema version 5 added the declared
route kind and portable mode and stored a deployment link's target string as
its fingerprint; version 4 documents remain readable, with those fields
